...
model.compile(loss=NLL, optimizer=sgd, metrics=['accuracy'])

x_test = data[:len(data)/4]
y_test = label[:len(label)/4]

x_train = data[len(data)/4:]
y_train = label[len(label)/4:]
...
history = model.fit(x_train, y_train, nb_epoch=100, batch_size=256, verbose=1)
